...
export class CartItemIdService {
  ...
  async findOne(id: number): Promise<CartItem> {
    const cartItem = await this.cartItemRepository.findOne({ where: { cart_item_id: id } });
    if (!cartItem) throw new NotFoundException(`Cart item with ID ${id} not found`);
    return cartItem;
  }

  async update(id: number, updateCartItemIdDto: UpdateCartItemIdDto): Promise<CartItem> {
  ...
  async getCartItemsByCartId(cartId: number): Promise<CartItem[]> {
    return await this.cartItemRepository.find({ where: { cart: { cart_id: cartId } } });
  }
}